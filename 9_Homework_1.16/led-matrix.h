...
public:
  ...
  bool SetPWMBits(uint8_t value); // [{ (getter) value에 입력된 PWM bit 값이 사용가능
                                  // 범위(1 ~ kBitPlane?? -> 뭔지는 모르겠지만 아마 하드웨어가 가질 수 있는
                                  // 최대 PWM bit값이겠지)내의 값인지 여부를 return하는 bool}]
  uint8_t pwmbits(); // [{ (setter) value값을 return }]

  /* Map brightness of output linearly to input with CIE1931 profile. */

  void set_luminance_correct(bool on); // [{ (getter) 아마도 cols rows등등의
                                       // 각종 디바이스 기본값들을 코딩 변수에 넣어주는??}]
  bool luminance_correct() const; // [{ (setter) on 값을 return}]

  void SetBrightness(uint8_t brightness); // [{ (getter) 생성된 모든 FrameCanvas의 밝기를 세팅함:1%~100% 
                                          // 하지만 FrameCanvas가 뭔지 도저히 못찾겠다 이는 현웅이가 찾아줄 것이다 }] 
  uint8_t brightness(); // [{ (setter) brightness 값을 return }]
  ...
  void Serialize(const char **data, size_t *len) const; // [{ Canvas의 정보를 저장해두는 역할인듯.
                                                        // bitplane_buffer_??를 *data에 복사,
                                                        // buffer_size_??를 *len에 복사 }]
  ...
  bool Deserialize(const char *data, size_t len); // [{ 저장된 Canvas의 정보를 가져오는 역할인듯.
                                                  // Serialize가 안돼있으면 false를 return,
                                                  // 돼있으면 *data를 bitplane_buffer_??에 복사하고 true를 return }]

  /* Copy content from other FrameCanvas owned by the same RGBMatrix. */
 
  void CopyFrom(const FrameCanvas &other); // [{other에 해당되는 FrameCanvas정보를 복사해온다 }]

  /* -- Canvas interface.*/
  
  virtual int width() const; // [{ virtual은 딸클래스가 override할 여지를 주는 뜻 }]
                             // [{ 이 줄 이후로 Canvas의 각종 설정값들 나열 }]
  virtual int height() const;
  virtual void SetPixel(int x, int y,
                        uint8_t red, uint8_t green, uint8_t blue);
  virtual void Clear(); // [{ 아무래도 초기화겠지? }]
  virtual void Fill(uint8_t red, uint8_t green, uint8_t blue); // [{ unit8_t는 특정 크기의 int, 아마 2^8인듯
                                                               // 그래서 0~255의 rgb값 설정을 할 수 있게 함 }]

private:
  friend class RGBMatrix; // [{ 나(FrameCanvas클래스)의 private를 RGBMatrix클래스와 공유한다 }]

  FrameCanvas(internal::Framebuffer *frame) : frame_(frame){} // [{ 생성자: frame이란 이름의 Framebuffer 클래스를 
                                                              // 생성하나봄. Framebuffer의 member는 rows,
                                                              // columns, parallel, scan_mode등등임. frame = frame_ }]
  virtual ~FrameCanvas();   // Any FrameCanvas is owned by RGBMatrix.
                            // [{ 소멸자 }]
  internal::Framebuffer *framebuffer() { return frame_; } // [{ framebuffer라는 이름의 Framebuffer클래스를 생성하고
                                                          // frame을 return?? }]

  internal::Framebuffer *const frame_; // [{ frame의 상수화 }]
}; // [{ 여기까지다. 내가 무슨 개소리를 지껄인건지 나도 모르지만 어쨋든 최선을 다했다. }]
